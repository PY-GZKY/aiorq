...
from aiorq.constants import default_queue_name
from aiorq.jobs import Job, JobDef, SerializationError
from aiorq.utils import timestamp_ms
from aiorq.worker import Retry, Worker, func


async def test_enqueue_job(aio_redis: AioRedis, worker):
    async def foobar(ctx):
        return 42

    j = await aio_redis.enqueue_job('foobar')
...
async def test_enqueue_job_different_queues(aio_redis: AioRedis, worker):
    async def foobar(ctx):
        return 42
...
async def test_enqueue_job_nested(aio_redis: AioRedis, worker):
    async def foobar(ctx):
        return 42
    ...
    job = await aio_redis.enqueue_job('parent_job')
    ...
    inner_job = Job(result, aio_redis)
    inner_result = await inner_job.result(poll_delay=0)
    assert inner_result == 42


async def test_enqueue_job_nested_custom_serializer(aio_redis_msgpack: AioRedis, worker):
    async def foobar(ctx):
        return 42
    ...
    job = await aio_redis_msgpack.enqueue_job('parent_job')
    ...
    result = await job.result(poll_delay=0)
    assert result is not None
    inner_job = Job(result, aio_redis_msgpack, _deserializer=functools.partial(msgpack.unpackb, raw=False))
    inner_result = await inner_job.result(poll_delay=0)
    assert inner_result == 42


async def test_enqueue_job_custom_queue(aio_redis: AioRedis, worker):
    async def foobar(ctx):
        return 42
    ...
    inner_job = Job(inner_job_id, aio_redis, _queue_name='spanner')
    inner_result = await inner_job.result(poll_delay=0)
    assert inner_result == 42


async def test_job_error(aio_redis: AioRedis, worker):
    async def foobar(ctx):
...
async def test_job_info(aio_redis: AioRedis):
    t_before = time()
    j = await aio_redis.enqueue_job('foobar', 123, a=456)
    info = await j.info()
...
async def test_mung(aio_redis: AioRedis, worker):
    """
...
async def test_custom_try(aio_redis: AioRedis, worker):
    async def foobar(ctx):
        return ctx['job_try']

    j1 = await aio_redis.enqueue_job('foobar')
...
async def test_custom_try2(aio_redis: AioRedis, worker):
    async def foobar(ctx):
        if ctx['job_try'] == 3:
            raise Retry()
        return ctx['job_try']
...
async def test_cant_pickle_arg(aio_redis: AioRedis, worker):
    class Foobar:
        def __getstate__(self):
            raise TypeError("this doesn't pickle")

    with pytest.raises(SerializationError, match='unable to serialize job "foobar"'):
        await aio_redis.enqueue_job('foobar', Foobar())


async def test_cant_pickle_result(aio_redis: AioRedis, worker):
    class Foobar:
        def __getstate__(self):
            raise TypeError("this doesn't pickle")
...
async def test_get_jobs(aio_redis: AioRedis):
    await aio_redis.enqueue_job('foobar', a=1, b=2, c=3)
    await asyncio.sleep(0.01)
    await aio_redis.enqueue_job('second', 4, b=5, c=6)
    await asyncio.sleep(0.01)
    await aio_redis.enqueue_job('third', 7, b=8)
    jobs = await aio_redis.queued_jobs()
